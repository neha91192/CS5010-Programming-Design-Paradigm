 ...
 * whitespace provided in user answer and considers options in the answer in the correct ascending
 * order and the answer must contain valid option number. Also, while evaluating answers, this
 * implementation will not regard any duplicate options.
 */
public class MultipleAnswersQuestion extends AbstractQuestion {
   ...
   * Answer Questions are equal if they are of same instance, their Question Text is exactly the
   * same and options also matches exactly with each other.
   * <p>This method immediately returns false if incoming object is not of type {@link
   * AbstractQuestion} and true if two objects are the same.</p>
   * <p>Otherwise it checks if the object is an instance of MultipleChoiceQuestion by calling
   * {@link MultipleAnswersQuestion#equalsMultipleAnswerQuestion(MultipleAnswersQuestion)} and then
   * checks if comparing two question texts returns 0. The output of these two condition is
   * returned.</p>
   *
  ...
  public boolean equals(Object object) {
    ...
    if (question.equalsMultipleAnswerQuestion(this)) {
      MultipleAnswersQuestion multipleAnswers = (MultipleAnswersQuestion) question;
      return multipleAnswers.getQuestionText().equals(this.getQuestionText())
              && Arrays.equals(options, multipleAnswers.options);
    }
    return false;
  }

  /**
   * Overriding hashcode implementation. Considering Question Text, options to evaluate hashcode in
   * this implementation.
   *
