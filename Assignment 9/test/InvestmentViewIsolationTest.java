...
/**
 * Tests for {@link InvestmentTextView}. It checks if the View class is working fine in isolation
 * with the Mock View.
 * <p>This test class checks for the correctness of View. It checks for each methods that
 * is being called through the view to a controller called "InvestmentController" See {@link
 * MockView} for more details.</p>
 * <p>First, it checks for the validity of the portfolio that is being done by any {@link
 * howtoinvestfordummies.model.InvestmentStrategies} implementation. If view is correctly passing
 * values to the controller that is the implementation of {@link InvestmentController} and
 * transmitting the log operations set by the controller, then view is working as expected in this
 * test.</p>
 */
public class InvestmentViewIsolationTest {
  /**
   * AppState is set to started, when app is already started. View should set the same string.
   */
