...
public class DetermineCostAndValue extends StockInvestmentCommandImpl {
  ...
  /**
   * This method takes model object to fetch cost and investment values of portfolio
   * with a required
   * name. If the portfolio name is unique, it will allow user to see values in the
   * portfolio.
   *
  ...
  public void execute(StockInvestmentStrategies model) {
    view.transmitMessage("Enter 'l' to list high level portfolio for which you want to "
            + "find cost and"
    ...
    while (true) {
      ...
      if (input.equalsIgnoreCase("l")) {
        String list = formatPortfolioNames(model.retrieveInvestmentStrategies("user"));
