...
public class RemoteStockAdapterImpl extends AbstractStockAdapter {
  ...
  /**
   * Fetches lowest stock price returned by
   * {@link RemoteStockAdapterImpl#getDataFromAPI(String)}
  ...
  public double getStockPriceForBuying(String stockSymbol, Date date) throws Exception {
    ...
    saveDataToLocalFile(stockSymbol, result);
  ...
  /**
   * Fetches closing stock price returned by
   * {@link RemoteStockAdapterImpl#getDataFromAPI(String)}
   * method for a given date.
   *
   * @param stockSymbol unique symbol for the stock company in String.
  ...
  private void saveDataToLocalFile(String name, String data) {
    String stockFile = "res/" + name + ".csv";
    try {
